...
Contexts of assemblies are built as iterated sums, the same way as contexts of sets.
...
In \cref{example:context} we placed a hypothesis into the context, but for this to make sense it needs to be a type family. How can a proposition be a type?
...
In \cref{cha:realizability-logic} we gave a realizability interpretation of logic, with its own notions of propositions and predicates. We show that it is equivalent to the truncated logic of the present chapter.
...
\begin{exercise}
  In the proof of \cref{thm:pred-equiv-pred} the passage from $P$ to the realizability predicate $p_P$ works for an arbitrary family~$P \in \Fam{S}$. Therefore, the equivalence extends to a pair of functors
  %
  \begin{equation*}
    \xymatrix{
      {\Pred{S}}
