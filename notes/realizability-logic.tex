...
It is customary to write $\R{r} \rz p x$ instead of $\R{r} \in p x$ and to read this as ``$\R{r}$ realizes $p x$''. Realizability predicates are more informative than Boolean predicates. The latter only express truth and falsehood, whereas the former provide \emph{computational evidence} for validity of statements.
...
Thus, $\R{i}$ converts computational evidence of $p x$ to computational evidence of~$q x$. Note that $\R{i}$ receives as input both a realizer for~$x$ and the evidence of~$p x$.
...
\begin{proof}
  ...
  %
  Given an assembly map $f : S \to \nabla \two$, define $p_f \in \Pred{S}$ by $\T{p_f} = \unit$ and
  % 
  \begin{equation*}
    \R{r} \rz p_f x
    \iff
