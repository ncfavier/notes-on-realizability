...
\begin{exercise}
  ...
  \end{equation*}
  % 
  Verify that $\pair{\place, \place}$ is a homeomorphism between $\Scott \times \Scott$ and
  $\Scott$.
\end{exercise}
...
\begin{proof}
  ...
  properties. See~\sidecite{Longley:95} for details.
\end{proof}
...
see~\cite{Longley:95} for details.
...
The closed terms of combinatory logic are generated from constants $\combK$, $\combS$ and a binary operation~$\cdot$.
...
In a pca the natural numbers may be encoded with the basic combinators as the Curry numerals. In a tpca they must be postulated separately.
...
\begin{proof}
  Similarly to the untyped case we define $\tpcalam{x}{t} e$ recursively as follows:
  %
  \begin{enumerate}
...
the equivalence relation generated by the equational rules of the simply typed
