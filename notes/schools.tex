...
\cref{sec:type-1}. In our setting it corresponds to the
...
develop a version of computable analysis. One gets an unusual variant which is a rich source
of counter-examples.
...
\begin{theorem}[Embedding Theorem for $\Scott$]
  ...
  A space $X$ may be pre-embedded in~$\Scott$ if, and only if, it is
  countably based.
\end{theorem}
...
will always denote the subbasic neighborhood filter.
